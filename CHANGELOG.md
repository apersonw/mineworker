...
这一版**只有一条行为修复**，其余是把一批我自己写错的性能结论更正过来 ——
包括 0.18.0 CHANGELOG 里的一句。

### 修复

- **分片后 cookie jar 可能比它的连接池先被换出。** `USE_SESSION` + 代理池下，
  同一个代理的多个分片本该共用一个 `CookieJar`（0.17.0 起的分片就是这么设计的），
  但 jar 缓存的容量只有连接池缓存的 1/片数 —— 代理数一多，jar 先被换出、
  连接池还活着，之后为同一代理新建分片就会拿到一个全新的 jar，
  **登录态在分片之间丢失**。

    修法要两个条件：两个缓存容量一致，且每次取连接池都刷新 jar 的 LRU 位置
    （只在新建时刷新的话，一个很热但从不重建的连接池，它的 jar 照样会变冷被挤掉）。
    影响 httpx 与 curl 两个同步下载器。

### 文档（都是更正，不改行为）

- **⚠️ 更正 0.18.0 里的一句错话**：那一版写着「即便分片之后，`DOWNLOADER_ASYNC`
  的优势也只在中等线程数（≤32）成立」。**那是错的** —— 它出自一个不公平的对照
  （同步侧没开 session，而异步下载器内部总是共享 client）。两边都开 session 复量：

    | 线程 | 8 | 16 | 32 | 64 | 128 |
    |---|---:|---:|---:|---:|---:|
    | async/sync | 0.97× | 0.99× | 0.84× | 0.48× | 0.49× |

    **公平比较下异步在任何线程数都没有优势。** 事件循环分片的价值是把异步从
    「>24 线程坍塌」救回到与同步同一量级，不是让它反超。

- **按代理缓存 client 的收益不是「连接复用」。** 同一套 A/B 换一个真会保活的代理
  （squid）重量，8/16/32 线程 1.23× / 1.87× / 2.22×，而在端到端都不保活的代理下是
  1.21× / 1.87× / 2.13× —— **几乎一样**。真正省掉的是每请求构造 / 销毁
  `httpx.Client`。修复本身没有变化，改的只是解释。

- **`pool_limits` 的依据换成真实的那组。** 旧文档引用的「281 → 499 QPS」是在一个
  不保活的代理后面量的，已证伪（两档连接数完全一样、QPS 只差 1.06×、分布重叠）。
  真实依据是直连：keepalive=20 时靶子接受 16 条连接、581 QPS；=100 时 5 条、1075 QPS。

- **两个分片阈值三环境复量。** `SESSION_SHARD_THREADS=32` 的拐点在直连 / squid /
  tinyproxy 下完全一致（峰都在 32），默认值站得住；`CURL_SESSION_SHARD_THREADS=16`
  的平台期确实从 ~16 开始，但 16-48 线程统计上分不开，措辞下调为「平台期起点的
  合理选择」。curl 分片收益从文档里的 2.4× 更正为**实测 1.3-1.6×**。

### 测试

- 三个全栈集成用例**打开熔断**（此前基于一个错误假设关掉了它），
  并新增「熔断跳闸后延迟但不丢页面」的用例 —— 两半都验：正常页面一个不少，
  且冷却真的生效。

## [0.18.0] - 2026-09-10

两条新增都是**静默错配**：框架有全部信息却不说话，后果只表现为「结果不太对」。

### 新增

- **分布式跑着进程内去重时会告警。** `DEDUP_FILTER` 默认是 `memory`（进程内布隆），
  分布式下每个节点各有一份指纹、互相不知道：两个节点解析出同一个 URL 会**重复抓**
  （实测两节点、两个入口页都链到同一页面：`memory` 下抓 2 次、`redis` 下 1 次），
  任务被重放时（租约是「至少一次」语义）会**重复入库** ——
  SQL 管道有唯一键兜底，CSV / Mongo / 自定义管道没有。

    共享队列保证「入了队的任务只被取走一次」，所以大部分现象看起来正常，
    这个错配此前不报错、只是悄悄多抓多写。现在节点启动时会提示改用
    `redis` / `redis-set`。`docs/distributed.md` 的示例里本来就是这么写的。

- **配置名写错时会提醒。** `setting.py` 或 `__custom_setting__` 里写了
  `DOWNLOAD_DELY`、`PROXY_ENABLED`、`ITEM_PIPELINE` 这类「差一点」的名字时，
  此前会被**静默接受、完全不生效**；现在会警告并给出最接近的真配置名。

    只在「像是拼错」时出声（`difflib`，cutoff 0.8）—— 用户在 `setting.py` 里
    定义自己的配置（`MY_API_KEY`、`SHOP_ID`）给爬虫读是正常用法，不会被打扰。
    值仍然照常设进去。

### 测试

- **新增三组「全特性同开」的集成用例**，判据全取框架**外部**（靶场收到哪些请求、
  真库落了几行、任务表最终状态），而不是看框架自己的统计：
  AirSpider（robots × 去重 × 重试 × 限速 × gzip × 真 PostgreSQL）、
  分布式 Spider（三进程并发写同一张表）、
  BatchSpider（master + 两 worker，补上此前没覆盖的**失败任务归宿**
  —— 页面永远 500 的任务必须落在「失败」，不能被谎报成「完成」——
  以及**数据真进 MySQL**的整链）。
...
- **异步下载器的事件循环按线程分片。** 「一个事件循环线程 + N 个工作线程阻塞提交」
  这个模式在 N 超过 ~24 时会**坍塌**：均在途从 18 掉到 4，请求近乎串行。

    新增 `ASYNC_THREADS_PER_LOOP`（默认 `16`）：每个事件循环最多服务这么多工作线程，
    超出再开一个。线程数不超过它时只开一个循环，**与分片前完全一致**
    （框架默认 `SPIDER_THREAD_COUNT=4` 落在这一档）。设成 `0` 关闭分片。

    | 线程 | 8 | 16 | 24 | 32 | 48 | 200 |
    |---|---:|---:|---:|---:|---:|---:|
    | 单循环 | 145 | 287 | 286 | **82** | **62** | **73** |
    | 分片后 | 145 | 287 | 417 | **452** | **318** | **421** |

    坍塌**与框架逻辑无关**：把框架整个拿掉、只留一个事件循环加 N 个线程反复
    `run_coroutine_threadsafe(...).result()`，坍塌一模一样。所以只能多开循环。

    ⚠️ **下面这句在 0.19.0 里更正了**：它出自不公平的对照（同步侧没开 session），
    两边都开 session 复量后，异步在任何线程数都没有优势。
    ~~即便分片之后，`DOWNLOADER_ASYNC` 的优势也只在中等线程数（≤32）成立；
    48 线程往上与同步下载器接近，200 线程时同步略胜。~~

- **curl 下载器的 session 也按线程分片。** 共用一个 `curl_cffi.Session` 时，
  吞吐从 16 线程起就不再增长 —— 32 线程往上，开 `use_session` 比不开还慢。

    新增 `CURL_SESSION_SHARD_THREADS`（默认 `16`）。**拐点与 httpx 那个是分别量出来的**：
    httpx 在 32、curl 在 16，共用同一个数字会让 32 线程那格 `ceil(32/32)=1`，等于没分片。
...
- **改正：`use_session` 在 curl 下载器上不带来连接复用。** 框架的 curl 路径永远用
  `stream=True`（响应体上限要边读边判），而 curl_cffi 收尾时会 `self.curl.close()`
  关掉整个句柄，连接缓存随之消失。实测保活连接：`stream=False` 2 条、
  `stream=True` **0 条**（`impersonate` 开关无影响，httpx 对照 2 条）。

    所以 `use_session=True` 在 curl 这边**只带来 cookie 持久化**。
    早先「三个下载器都复用连接」的说法对 curl 是错的。要拿回复用得放弃
    `MAX_RESPONSE_SIZE` 的边读边判，那是安全边界，不做这个交换。

- 改正 `DOWNLOADER_ASYNC_CONCURRENCY` 的描述。它此前被写成「async 下载器最大在途
  请求数」，**那是错的**：工作线程是同步阻塞地调下载器的，实际在途由
  `SPIDER_THREAD_COUNT` 决定，实测默认 200 时均在途只有 3~23，这个值从没成为过约束。
...
- **连接池按线程分片 —— 修掉 0.16.0 记录的那个负收益区。** 一个 `httpx.Client`
  被太多线程共用时，连接池自己成为争用点：共用一个 client 的吞吐到 ~32 线程见顶后
  **掉头向下**，48 线程往上甚至比不开 `USE_SESSION` 还慢。

    新增 `SESSION_SHARD_THREADS`（默认 `32`，即实测拐点）：每个代理按需开多片连接池。
    线程数不超过它时只有一片，**与分片前完全一致**（框架默认 `SPIDER_THREAD_COUNT=4`
    落在这一档，现有部署零影响）。设成 `0` 关闭分片。

    验收（https、单代理、每格 5 轮，靶子天花板已用裸 asyncio 客户端确认远高于被测）：

    | 线程 | 不开 session | 0.16.0 | 本次 |
    |---:|---:|---:|---:|
    | 16 | 146 | 289 | 284 |
    | 32 | 211 | 415 | 468 |
    ...
    **cookie 语义未变**：同一个代理的所有分片共用一个 `CookieJar`
    （httpx 收到裸 `CookieJar` 时按引用使用，且 `CookieJar` 自带锁），
    `USE_SESSION` 文档里「连同 cookie jar 一起复用」的承诺原样保住。
    不同代理之间仍然隔离，与分片前一致。

    只影响同步 httpx 下载器：异步下载器跑在单个事件循环里不存在这种线程争用，
    `curl_cffi` 用 libcurl 自己的连接池，机制不同，均未改动。


### 测试

- **脚手架的守卫拦不住任何真实故障。** `mineworker create` 生成的项目原来只做
  `ast.parse` 检查 —— 把模板里的 `mw.Request` 改成不存在的 `mw.RequestXXX`、
  或声明一个框架不认识的配置项，测试仍然全绿，而每个新用户生成的项目都跑不起来。

    现在真的把生成的爬虫模块 import 进来、实例化、并**迭代 `start_requests()`**
    （模板里的调用写在生成器体内，不迭代执行不到），
    并逐项核对 `setting.py` 模板里声明的配置在框架里都存在。不联网。

## [0.16.0] - 2026-09-09

### 修复

- **代理失败是永久放逐，不是冷却。** `report_bad` 把代理加进一个**只进不出**的集合，
  而拉列表时又拒绝放回集合里的 —— **一次瞬时错误就永久踢掉一个代理**。
  而瞬时错误是常态：压测里一个健康的本地 tinyproxy 在 960 个请求中也重置了
  3 条 keep-alive 连接。

    后果不是崩溃，是静默降级：池被慢慢啃空，配合 `PROXY_ALLOW_DIRECT=False`（默认），
    之后每个请求都先等满 `PROXY_WAIT_TIMEOUT`（默认 30 秒）再失败。

    改为冷却 + 指数退避（`PROXY_BAN_SECONDS` 起步，翻倍，封顶
    `PROXY_BAN_MAX_SECONDS`）。距上次失败超过封顶值即视为已恢复、退避重新计数。
    **`report_bad` 之后代理仍然立刻退出轮转**，只是不再是永久的。
    要回到旧行为把 `PROXY_BAN_SECONDS` 设成 0。

- **连接池缓存的「查—建—存」没有锁。** N 个线程首次碰到同一个代理会各建一个
  client（实测 32 线程建了 32 个），而其中 N-1 个被覆盖时**既没交还也没关闭**。
  改为原子的取或建，抢输的线程关掉自己刚建的那个。

- **`httpx` 连接池上限从没配过。** 用的是 httpx 默认的
  `max_connections=100` / `max_keepalive_connections=20` —— **两者不等**，
  并发一超过 20，多出来的连接每轮用完就被关掉、下轮重建。
  现在按并发数配置，两个值都只增不减（低于 httpx 默认值时按默认走）。
  异步下载器的主 client 一直配着上限、「每代理」那条却没有，一并补上。

### 新增

- `PROXY_BAN_SECONDS` / `PROXY_BAN_MAX_SECONDS` —— 代理失败后的冷却与退避上限。
- `benchmarks/proxylab.py` —— 压测用的 tinyproxy 启动器。

### 已知问题

- **「按代理复用连接」有一个负收益区。** 实测（https、单代理、每格 7 轮，
  靶子天花板已用裸 asyncio 客户端确认远高于被测）：
    ...
    共用 client 在 ~32 线程见顶后掉头向下，拐点在 32–48 线程之间。
    **线程数超过约 40 且代理很少时，`use_session` 比不开更慢。**
    默认 `SPIDER_THREAD_COUNT=4` 落在收益区。方向是按代理分片，
    但那会改变 cookie jar 的共享范围，需要单独设计，尚未修。

## [0.15.0] - 2026-09-09

### 修复

- **`UpdateItem` 同时声明 `__unique_key__` 时，后续更新会被去重静默丢掉。**
  指纹按 `__unique_key__` 算的话，第二次更新和第一次同指纹 ——
  而 `UpdateItem` 的语义正是「同一条记录、**新的值**」，去重默认还是开着的。

    实测（同一个 sku，价格 100 → 120 → 150）：只有 **100** 写了出去。
    两个功能各自都在文档里、各自都对，**合起来才出问题** ——
    而对 `UpdateItem` 来说把 sku 声明成唯一键是再自然不过的事。

    现在 `UpdateItem` 的指纹**永远按全部非空字段算**：值变了写得进去，
    值没变的空转重复仍然被挡住。`update_key` 不受影响 ——
    不写 `__update_key__` 时仍会回退到 `unique_key`（那是「怎么写」，
    不是「要不要写」）。
