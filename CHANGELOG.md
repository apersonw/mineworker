...
- **改正：按代理缓存 client 的收益不是「连接复用」。** 同一套 A/B 换一个真会保活的
  代理（squid）重量，8/16/32 线程分别是 1.23× / 1.87× / 2.22×，
  而在端到端都不保活的代理下是 1.21× / 1.87× / 2.13× —— **几乎一样**。
  真正省掉的是每请求构造 / 销毁 `httpx.Client`；单线程下这笔账 5.1ms/请求，
  16 线程下约 50ms/请求，并发把它放大了一个数量级（放大机制尚未隔离）。

    修复本身没有变化，改的只是解释。`use_session=True` 直连时确实会复用连接
    （6 个请求只开 1 条，见 `tests/test_connection_reuse_truth.py`）。
