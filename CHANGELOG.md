...
**因为是全新的 PyPI 项目，版本号从 1.0.0 重新起**（`v0.1.0`……`v0.23.0` 这些
git tag 已经是改名前 mineworker 自己的历史版本，占用了；1.0.0 也更准确——
这不是一个刚起步的 0.x 项目，是同一套跑过真实生产的代码换了个名字），
不延续改名前的 0.23.0。
...
49 小时里 1160 个实例有 **1158 个「请求成功 0，入库 0 条」**，全部 exit 0、
全部被记成 success。只有第一轮真干了活。

不是哪一行代码错了，是两个各自正确的模型对不上：框架里「一个 `redis_key` =
一个可续跑的作业」（种子只注入一次、去重永久），平台里「每次触发 = 一次全新运行」。
第一轮留下的 `lock:seed`（24 小时 TTL）让之后每个节点都走「另一节点已注入种子」，
每 24 小时抢到锁的那一个又被无 TTL 的布隆把种子 URL 挡掉。

### 新增

- **`RUN_ID`（环境变量 `MINEWORKER_RUN_ID`）：运行作用域。** 设了之后队列 /
  种子锁 / 在途 / 心跳 / 失败列表都落到 `<prefix>:<redis_key>:run:<id>` 下，
  每次运行互不相干；同一个 id 再起 = 续那次运行（断点续爬的语义没丢，只是挪到了
  运行这一层）。MineWorkerHub 会给每个实例自动注入。
- **`DEDUP_SCOPE`（`auto` / `run` / `spider`）：去重落在哪。** `run` = 每次运行
  从头抓（定时重抓要的），`spider` = 跨运行持久（增量爬）。默认 `auto`：有 `RUN_ID`
  就是 `run`。**默认偏向 `run`** —— 选错的代价不对称：`run` 选错了是多抓一遍
  （看得见），`spider` 选错了是空转（exit 0，看不见）。显式选 `spider` 时启动会打
  一行 INFO 说明本轮不会重抓。Item 去重也跟着运行走 —— 否则「请求成功 63、入库 0」
  是另一种空转。
- **`RUN_TTL`（默认 7 天）：运行作用域下 key 的保留时间。** 跑着时由心跳续期
  （8 天的运行不会在第 7 天被自己清掉），结束后到期自动清理 —— 否则每 5 分钟
  一次的定时任务一天就在 Redis 里留下 288 套 key。另有 `<prefix>:<redis_key>:runs`
  索引（zset），监控不必 SCAN。
- **没设 `RUN_ID` 的老部署，空转也不再是静默的。** 节点拿不到种子锁时看一眼：
  锁是很早以前留下的、队列空、没有在途、没有活节点 —— 那是个已完成的作业，
  打一条 **WARNING** 说明本轮无事可做、以及怎么改（设 `RUN_ID` / 换 `redis_key` /
  删锁）。同时启动的节点之间不会误报（锁太新）。

### 修复

- 测试隔离：`set_effective_concurrency` 的全局在用例之间没有重置，
  `test_session_sharding` 一直只是**碰巧**绿的（按字母序排在它前面的用例顺手清了）。
  现在 conftest 每例前后都清。

## [0.20.0] - 2026-09-10

**开代理池的用户建议尽快升级。** 这一版修的五条全是**静默故障** ——
行为、日志、返回值都正常，只是没按设计工作：

- 一个响应慢的 URL 就能把整池健康代理清空，之后每个请求空等 30 秒；
- 代理用满次数被换出后再也回不来，池子跑一阵子退化成每秒几个；
- 按代理做键的记账只增不减，用轮换型代理源会一直吃内存；
- 「第 N 次**连续**失败」这句日志是假的，数的是累计；
- `Spider(thread_count=N)` 从来没传到分片逻辑里，白丢 1.65×/3.9× 吞吐。

**前四条都要跑一阵子才现形** —— 短测试和短试用看不见，正好是上生产之前
最该修掉的那一类。

另外这一版还**推翻了 0.19.0 里我自己刚写下的一条性能结论**（`USE_SESSION`
的收益就是连接复用），并还清了挂了两个发布版的一笔账（并发放大的机制是
GIL 串行，倍数就是线程数）。

### 修复

- **代理用满 `PROXY_MAX_USE_TIMES` 之后能轮换回来了。** 计数原先从不清零，
  被丢出池的代理补回来**再用一次就又被丢**。后果只在第 `用满次数 × 池大小`
  个请求之后出现（默认 100×N），短测试看不见：实测 3 个代理的池在第一轮之后
  稳态退化成**每秒只取到 3 个**（其余空手而归，各自等满 `PROXY_WAIT_TIMEOUT`），
  并且**每秒去拉一次供应商的列表接口，永远不停**。修后同配置每秒 15 个。
- **代理池的记账不再无限增长。** `_use_count` / `_banned` / `_fails` /
  `_suspects` 都以代理为键，而很多代理商每次返回全新 IP —— 实测 3000 次
  `get_proxy` 之后 `_use_count` 里躺着 1500 条，只增不减。现在每次拉取列表前
  回收一次，且只做语义等价的回收，**退避阶梯不受影响**（有用例盯着）。
- **目标站的错不再拉黑代理。** 下载器原先把任何下载异常都当成代理故障。
  实测：三个**完全健康**的代理（各自都成功连上并转发了请求）配一个 5 秒才回的
  目标站、1 秒超时 —— **三个请求就把整池清空**，之后每个请求都要等满
  `PROXY_WAIT_TIMEOUT`（默认 30 秒）才失败。一个慢 URL 让整个爬虫停摆一分钟。
  现在按阶段分流：连不上代理仍然立刻拉黑；连上之后的失败（读超时、连接重置、
  响应畸形）记「可疑」，连续攒够 `PROXY_SUSPECT_BAN_AFTER`（默认 3）次才拉黑。
- **代理池现在知道「成功」了。** 此前 `_fails` 只在距上次失败超过 900 秒时清零 ——
  日志里那句「第 N 次**连续**失败」是假的，数的是 15 分钟内的累计失败，
  一个跑了一万次成功、偶尔失败三次的代理会被退避到 4 倍冷却。
  新增可选钩子 `report_good`，成功一次即清零。
  **升级前写的自定义代理池不受影响** —— 三个新钩子都是可选的（`getattr` 取，
  没有就跳过）；这条是被仓库自己的 `StaticPool` 在全栈用例里撞红后补上的。
- **`Spider(thread_count=N)` 现在会传到下载器的分片逻辑。** 此前
  `pool_limits` / `shard_count` / `loop_count` 直接读 `setting.SPIDER_THREAD_COUNT`，
  而 `thread_count=` 只改工作线程数 —— 于是 `AirSpider(thread_count=64)` 配默认的
  `4` 会算出 **1 片、1 个事件循环**，v4.34/v4.35 的分片修复静默不生效，
  且日志与行为完全正常。代价实测（同样 64 个真实线程、50ms 靶子）：
  同步 **546 → 899**、异步 **198 → 775** QPS。
  多个爬虫同进程时取最大值，爬虫收尾时随下载器一起重置。

### 文档

- **那笔账：放大倍数就是线程数。** `httpx.Client` 的构造被 GIL 完全串行化 ——
  聚合吞吐与线程数无关（1→32 线程 1.03×），单次墙钟延迟正比于线程数
  （0.51 → 9.90 ms）。带阴阳两个对照：纯 CPU 计算呈现同样形状（1.09×），
  `sleep` 不呈现（29.42×），所以这个形状不是线程开销本身带来的。

- **⚠️ 推翻 0.19.0 的一条：`USE_SESSION` 的收益就是连接复用。**
  把「每条连接值多少钱」当旋钮拧（32 线程、直连），收益单调跟着涨：
  0ms → 1.10×、2ms → 1.15×、10ms → 1.28×、50ms → 1.84×。**形状本身就是机制** ——
  开着 session 的那一臂几乎不动（483/488/485/472），涨上去的全是不开的那一臂
  在掉（440→256）。构造成本在这四档里完全相同，产生不出这个形状。
  **但 0.19.0 引用的那两组代理数字为什么相等，本轮没能解释**，如实留白。

- **新增「每进程的开销天花板」一节。** 固定 32 线程逐层剥：裸 socket 15,036 QPS
  → 换成共享 `httpx.Client` **534** → 框架下载器 487。**一层掉 28 倍，
  框架只加 10%。** 三个后端里只有 `curl_cffi` 随线程数上涨（635→978），
  但它每个请求烧一个端口（600 请求 → 616 个 TIME_WAIT，httpx 开 session 只有 42），
  持续几百 QPS 会打光临时端口。选型建议见 `docs/settings.md`。

- **否掉一个旋钮**：`sys.setswitchinterval` 在 1000 倍范围内对 httpx 无效
  （0.93~1.01×），不是车队效应，默认值已是最优 —— 不加这个配置。

- **`async-kernel.md` 里「`USE_SESSION` 是个死配置」标为已修**，并给
  「线程越多越慢」那条曲线补上了归因。

### 测试

- `tests/test_connection_reuse_truth.py` 新增两条，数**客户端→代理**那条腿的
  连接数（开 session 1 条 / 6 请求，不开 6 条 / 6 请求）。此前所有复用用例数的
  都是直连 —— 而 0.19.0 那条错结论正是在代理环境下得出的。
...
- ~~**按代理缓存 client 的收益不是「连接复用」。**~~ 同一套 A/B 换一个真会保活的代理
  ...
  `httpx.Client`。~~修复本身没有变化，改的只是解释。
    ...
    > ⚠️ **这一条在 0.20.0 里被推翻了**：收益就是连接复用。上面这两组数字是真的，
    > 但推不出那个结论，而那两组数字为什么相等至今没有解释。见下一版。
