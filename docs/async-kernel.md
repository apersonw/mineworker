...
!!! warning "这一节原来的标题是「每请求重建连接」，那个归因是错的"
    这个项目在同一个归因上错过两次：

    1. §5(a) 查明 3.8× 的真凶是 **`SSLContext` 构造**（32.9ms/个），
       不是重新建立 TCP 连接；
    2. 后来给「按代理缓存 client」量收益时又写成「连接复用」——
       同一套 A/B 换一个**真会保活**的代理重量，数字几乎一样
       （详见 `settings.md` 的「这个收益不是连接复用」）。

    共同的教训：**「省掉了每请求新建 Client」和「复用了连接」是两件事**，
    前者才是这里省下的东西。
...
!!! note "差距已大幅缩小（v4.34 之后复量，5 轮，均开 session）"
    ...
    **方向没变，倍数变了**：当年是「同步快 4–10 倍」，现在 ≤16 线程基本持平、
    ...
    [事件循环分片](../docs/settings.md)（单个事件循环在 >24 线程时会坍塌）。
