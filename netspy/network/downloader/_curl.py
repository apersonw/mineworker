...
def _requests() -> Any:
    """延迟导入：curl_cffi 是可选依赖，没装也不该影响 import netspy。"""
    try:
        from curl_cffi import requests
    except ModuleNotFoundError as exc:  # pragma: no cover - 取决于是否装了 extra
        raise RequestError('TLS 指纹伪装需要 curl_cffi，安装：pip install "netspy[curl]"') from exc
    return requests


class CurlDownloader(Downloader):
    """与 :class:`~netspy.network.downloader._httpx.HttpxDownloader` 同构，
    ...
    def _session_for_proxy(self, proxy: str | None) -> CurlSession:
        ...
        三种环境各跑 5 轮（32/48/64 线程，分片 vs 共用一个）：

            直连       1.49× / 1.34× / 1.63×
            squid      1.41× / 1.54× / 1.47×
            tinyproxy  1.16× / 1.43× / 1.66×

        一致在 **1.3-1.6×**。⚠️ 早先这里写的是「2.4×」，**那个幅度没有复现**
        （当时只在 tinyproxy 下量过一次）—— 收益是真的，倍数被高估了。

        **拐点比 httpx 低**：curl 共用一个 Session 从 ~16 线程起就不再增长
        （8→136、16→207、24→224、32→228、48→225 QPS，直连）。
        但 16 到 48 全在一个带里、离散 14-25%，**统计上分不开** ——
        所以 16 是「取在平台期起点」的合理选择，不是一个能精确指认的峰。
        httpx 那边不同：那里有清晰的峰（32 线程 486，48 线程掉到 282）。
