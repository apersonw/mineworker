"""框架异常层级。所有自定义异常均继承 `NetspyError`。"""

from __future__ import annotations


class NetspyError(Exception):
    """所有 Netspy 异常的基类。"""


class ConfigError(NetspyError):
    """配置缺失或非法。"""


class SpiderError(NetspyError):
    """爬虫定义或运行期错误。"""


class RequestError(NetspyError):
...
class HttpStatusError(RequestError):
    ...
    继承 `RequestError`，因此复用既有的下载失败重试路径与 `netspy retry` 回放。
    """
...
class ContentTypeRejectedError(NetspyError):
...
class ResponseError(NetspyError):
    """响应内容异常（解析失败、非预期结构等）。"""


class ValidationError(NetspyError):
    """`validate()` 校验不通过；触发重试或丢弃。"""


class NotRetryError(NetspyError):
    """在解析中抛出以放弃当前请求，跳过剩余重试。"""


class ItemError(NetspyError):
    """Item 定义或序列化错误。"""


class PipelineError(NetspyError):
    """管道写入失败。"""


class DedupError(NetspyError):
    """去重过滤器错误。"""
