"""脚手架生成：项目 / 爬虫 / Item / setting（`netspy create`）。"""
...
def _render(relpath: str, /, **ctx: Any) -> str:
    source = (
        importlib.resources.files("netspy.templates").joinpath(relpath).read_text(encoding="utf-8")
    )
...
def create_setting(*, target_dir: Path | None = None, force: bool = False) -> Path:
    ...
    _write(path, _render("setting.py.jinja", project_name="netspy"), force=force)
    return path
