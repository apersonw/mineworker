...
from netspy.dedup.lite_filter import LiteFilter
...
def _make_filter(
    ...
    *,
    ...
    if filter_type == "redis-set":
        from netspy.dedup.redis_filter import RedisSetFilter
...
def get_request_filter(name: str = "netspy:requests", redis_client: Redis | None = None) -> Dedup:
    """请求去重（fingerprint 已是 md5，不再二次 md5）。"""
    return Dedup(to_md5=False, name=name, redis_client=redis_client)


def get_item_filter(name: str = "netspy:items", redis_client: Redis | None = None) -> Dedup:
