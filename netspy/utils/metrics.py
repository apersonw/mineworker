...
Prometheus 需 ``pip install netspy[metrics]`` 且设 ``METRICS_PROMETHEUS_PORT > 0``。
"""
...
class _Prometheus:
    ...
    def update(self, values: dict[str, int]) -> None:
        for key, value in values.items():
            ...
            if gauge is None:
                gauge = self._gauge_cls(f"netspy_{key}", key, registry=self._registry)
...
def _make_prometheus(port: int) -> _Prometheus | None:
    ...
    try:
        ...
        log.warning("未安装 prometheus-client，跳过 exporter（pip install netspy[metrics]）")
        return None
