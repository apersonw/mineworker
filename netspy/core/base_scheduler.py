...
from netspy.buffer.item_buffer import ItemBuffer, ItemHandler
...
from netspy.network.proxy_pool import close_proxy_pool
...
from netspy.utils.stats import RUN_SUMMARY_MARKER, Stats
...
class BaseScheduler:
    ...
    def _emit_run_summary(self) -> None:
        """把机器可读摘要打到 stdout（一行 `NETSPY_RUN_SUMMARY {...}`）。
    ...
    def _on_failed_request(self, request: Request) -> None:
        ...
        落到 `FAILED_REQUEST_PATH`，等 ``netspy retry --requests`` 回放。
    ...
    def _replay_failed_requests(self) -> int:
        ...
        `netspy retry --requests` 只是探活（重新下载看状态码，不跑回调），
        数据要靠这条路才回得来。
    ...
    def _dump_requests(self, requests: list[Request], why: str) -> None:
        """把没能安置的请求落到 `FAILED_REQUEST_PATH`，等 ``netspy retry`` 回放。
