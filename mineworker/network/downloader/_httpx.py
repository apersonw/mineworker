...
class HttpxDownloader(Downloader):
    def __init__(
        ...
        *,
        ...
        #
        # 收益**就是连接复用**。这个归因在本项目里翻过三次面，这次有剂量反应曲线：
        # 把「每条连接值多少钱」当旋钮拧（32 线程，靶子对每条新连接收一次费）——
        #
        #     0ms → 1.10×    2ms → 1.15×    10ms → 1.28×    50ms → 1.84×
        #
        # 涨上去的全是**不开 session 那一臂在掉**（440→256 QPS），开着的一臂几乎不动
        # （483/488/485/472）—— 它不在乎建连贵不贵，因为它基本不建连（320 个请求
        # 只开 21~40 条）。构造成本在这四档里完全相同，产生不出这个形状。
        #
        # ⚠️ v4.40 曾据「squid 与 tinyproxy 下收益几乎一样」断定收益**不是**复用。
        # 上面这条曲线与那个结论矛盾，所以那个结论不成立。
        # 但**它那两组数字为什么相等，仍然没有解释**：实测 tinyproxy 连
        # 客户端→代理这条腿也基本不保活（不开 session 时 6 个请求 6 条连接），
        # 所以「两种代理对本连接池等价」这个本来顺理成章的解释是错的。
        # 走代理时的机制可能与直连不同 —— 没查清就不编，如实留白。
        # 直连这条腿的用例见 `tests/test_connection_reuse_truth.py::*_to_the_proxy`。
        #
        # 构造成本另有其账，且已经付过了：`ssl_context_for` 的缓存把它从 41ms 压到
        # 0.44ms（裸 `httpx.Client()` 每次重建 SSLContext）。剩下的 0.44ms 被 GIL
        # 完全串行化，放大倍数就是线程数（32 线程下单次墙钟 9.9ms），但在 ~500 QPS
        # 的量级上封不了顶 —— 封顶的是 httpx 自己的每请求 Python 开销，见 docs/settings.md。
        #
