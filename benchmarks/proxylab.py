...
6 个请求接受了 6 条连接，而直连只接受 1 条）。

**两条腿都不保活** —— v4.45 在框架与 tinyproxy 之间插了个透明 TCP 中继
数入站连接：不开 session 时也是 6 条 / 6 请求。所以「客户端→代理」这条腿
同样不能拿它量。要量复用请用 `squidlab.py`。做别的压测（并发、争用、代理开销）它仍然合适 ——
上面那组选型数字就是这么量的。
